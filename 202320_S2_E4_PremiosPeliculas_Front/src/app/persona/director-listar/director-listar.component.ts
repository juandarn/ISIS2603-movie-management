...
@Component({
  selector: 'app-director-listar',
  templateUrl: './director-listar.component.html',
  styleUrls: ['./director-listar.component.css']
})
export class DirectorListarComponent implements OnInit {
  
  rol!: string;
  directorSelecionado!: PersonaDetailDTO;
  selected: boolean = false;
  directores: Array<PersonaDetailDTO> = [];
  ...
  getDirectores() {
    this.personaService.getDirectores(this.orden).subscribe(apiData => {
      this.directores = apiData;
      this.cargarFiltro();
      this.filtros();
    });
  }
  ...
  cargarFiltro() {
    this.directores.forEach(persona => {
  ...
  filtros() {
    let directore: Array<PersonaDetailDTO> = [];
    this.directores.forEach(persona => {
      this.nacionalidadesUnicas.add(persona.nacionalidad);
      let add = true;
